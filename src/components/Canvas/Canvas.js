...
export default function Canvas({ searchParams }) {
  var FX, w, h;
  ...
  const draw = async (p5) => {
    ...
    FX = fx(searchParams.get('h') ? searchParams.get('h') : typeof searchParams.get('reset') === 'string');
    setFxHash(FX.hash);
    setRes(`(${w}x${h})`);
  ...
  return (
    <Sketch setup={setup} draw={draw} windowResized={windowResized} className={styles.canvasContainer} />
  );
};